# Linear process Model: AutoCov
# n = 100 and n = 200
# p = c(50, 75, 100)

library(rcov)
n = 100 # sample size n = 200
p = 20#c(50, 75, 100) # p = c(50, 75, 100) dimensionality
...
results = array(NA, dim = c(B, 3, 7, length(p), length(dist), 3))
...
for (k in 1:length(p)){
  ...
  Gamma0 = Gamma_pop(M.list = M.list, 0, rho)
  Gamma1 = Gamma_pop(M.list = M.list, 1, rho)
  Gamma2 = Gamma_pop(M.list = M.list, 2, rho)

  for (h in 1:length(dist)){
    for(b in 1:B){
      X = linear_simu(n = n, mu = rep(0, p[k]), M.list = M.list, rho = rho, K = 1001, err.dist = dist[h])
      ...
      results[b,1,1,k,h,1] = max(abs(temp.mat))
      results[b,1,1,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,1,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod0.mat, MARGIN = 1, median), nrow = p[k]) - Gamma0
      results[b,1,2,k,h,1] = max(abs(temp.mat))
      results[b,1,2,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,2,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod0.mat, MARGIN = 1, FUN = function(x){MoM_mean(x, 10)}), nrow = p[k]) - Gamma0
      results[b,1,3,k,h,1] = max(abs(temp.mat))
      results[b,1,3,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,3,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod0.mat, MARGIN = 1, FUN = function(x){MoM_mean(x, 20)}), nrow = p[k]) - Gamma0
      results[b,1,4,k,h,1] = max(abs(temp.mat))
      results[b,1,4,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,4,k,h,3] = norm(temp.mat, type = "F")

      tau1_cv.vec = optim.tuning(crossprod0.mat, S = 10, h = 1, M_est = F)
      tau2_cv.vec = optim.tuning(crossprod0.mat, S = 10, h = 1, M_est = T, max.tau = 100)
      ...
      results[b,1,5,k,h,1] = max(abs(temp.mat))
      results[b,1,5,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,5,k,h,3] = norm(temp.mat, type = "F")
      ...
      results[b,1,6,k,h,1] = max(abs(temp.mat))
      results[b,1,6,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,6,k,h,3] = norm(temp.mat, type = "F")
      ...
      results[b,1,7,k,h,1] = max(abs(temp.mat))
      results[b,1,7,k,h,2] = norm(temp.mat, type = "2")
      results[b,1,7,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod1.mat, MARGIN = 1, mean), nrow = p[k]) - Gamma1
      results[b,2,1,k,h,1] = max(abs(temp.mat))
      results[b,2,1,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,1,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod1.mat, MARGIN = 1, median), nrow = p[k]) - Gamma1
      results[b,2,2,k,h,1] = max(abs(temp.mat))
      results[b,2,2,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,2,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod1.mat, MARGIN = 1, FUN = function(x){MoM_mean(x, 10)}), nrow = p[k]) - Gamma1
      results[b,2,3,k,h,1] = max(abs(temp.mat))
      results[b,2,3,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,3,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod1.mat, MARGIN = 1, FUN = function(x){MoM_mean(x, 20)}), nrow = p[k]) - Gamma1
      results[b,2,4,k,h,1] = max(abs(temp.mat))
      results[b,2,4,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,4,k,h,3] = norm(temp.mat, type = "F")

      tau1_cv.vec = optim.tuning(crossprod1.mat, S = 10, h = 1, M_est = F)
      tau2_cv.vec = optim.tuning(crossprod1.mat, S = 10, h = 1, M_est = T, max.tau = 100)
      ...
      results[b,2,5,k,h,1] = max(abs(temp.mat))
      results[b,2,5,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,5,k,h,3] = norm(temp.mat, type = "F")
      ...
      results[b,2,6,k,h,1] = max(abs(temp.mat))
      results[b,2,6,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,6,k,h,3] = norm(temp.mat, type = "F")
      ...
      results[b,2,7,k,h,1] = max(abs(temp.mat))
      results[b,2,7,k,h,2] = norm(temp.mat, type = "2")
      results[b,2,7,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod2.mat, MARGIN = 1, mean), nrow = p[k]) - Gamma2
      results[b,3,1,k,h,1] = max(abs(temp.mat))
      results[b,3,1,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,1,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod2.mat, MARGIN = 1, median), nrow = p[k]) - Gamma2
      results[b,3,2,k,h,1] = max(abs(temp.mat))
      results[b,3,2,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,2,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod2.mat, MARGIN = 1, FUN = function(x){MoM_mean(x, 10)}), nrow = p[k]) - Gamma2
      results[b,3,3,k,h,1] = max(abs(temp.mat))
      results[b,3,3,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,3,k,h,3] = norm(temp.mat, type = "F")

      temp.mat = matrix(apply(crossprod2.mat, MARGIN = 1, FUN = function(x){MoM_mean(x, 20)}), nrow = p[k]) - Gamma2
      results[b,3,4,k,h,1] = max(abs(temp.mat))
      results[b,3,4,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,4,k,h,3] = norm(temp.mat, type = "F")

      tau1_cv.vec = optim.tuning(crossprod2.mat, S = 10, h = 1, M_est = F)
      tau2_cv.vec = optim.tuning(crossprod2.mat, S = 10, h = 1, M_est = T, max.tau = 100)
      ...
      results[b,3,5,k,h,1] = max(abs(temp.mat))
      results[b,3,5,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,5,k,h,3] = norm(temp.mat, type = "F")
      ...
      results[b,3,6,k,h,1] = max(abs(temp.mat))
      results[b,3,6,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,6,k,h,3] = norm(temp.mat, type = "F")
      ...
      results[b,3,7,k,h,1] = max(abs(temp.mat))
      results[b,3,7,k,h,2] = norm(temp.mat, type = "2")
      results[b,3,7,k,h,3] = norm(temp.mat, type = "F")
...
save(results, file = "simu_cov_lp.RData")
